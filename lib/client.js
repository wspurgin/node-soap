...
Client.prototype._initializeOptions = function(options) {
  var defaultChar = '_';
  var originalNormalizeChar = (options.normalizeChar || defaultChar);

  this.streamAllowed = options.stream;
  this.normalizeNames = options.normalizeNames;
  this.normalizeRegex = options.normalizeRegex;
  ...
  if (originalNormalizeChar && originalNormalizeChar.length === 1) {
    this.normalizeChar = originalNormalizeChar;
  } else {
    console.warn("node-soap: Given `normalizeChar` option '", originalNormalizeChar, "' which is longer than 1 character. Defaulting to '", defaultChar, "'");
    this.normalizeChar = defaultChar;
  }

  if (this.normalizeRegex && !(this.normalizeRegex instanceof RegExp)) {
    console.warn('node-soap: Given `normalizeRegex` option that is not an instance of RegExp. Ignoring');
    this.normalizeRegex = null;
  }
};
...
Client.prototype._definePort = function(port, endpoint) {
  ...
  for (var name in methods) {
    ...
    var methodName = this.normalizeNames ? name.replace(nonIdentifierChars, this.normalizeChar) : name;
    methodName = this.normalizeRegex ? name.replace(this.normalizeRegex, this.normalizeChar) : methodName;
